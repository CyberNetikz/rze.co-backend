...
class TradeExecutionService {
   ...
   */
  async executeTrade({ symbol, entryPrice, positionSize = null, templateId = null }) {
    const db = database.getDb();
    const tradeUuid = uuidv4();
    
    logger.trade('Starting new trade execution', { symbol, entryPrice, tradeUuid });
    ...
    try {
      ...
      const startingCapital = settings.starting_capital || account.portfolio_value;
      const tradeSizePercent = settings.trade_size_percent || 20;
      const calculatedPositionSize = positionSize || (startingCapital * (tradeSizePercent / 100));
      ...
      if (calculatedPositionSize > account.buying_power) {
        throw new Error(`Insufficient buying power. Need $${calculatedPositionSize.toFixed(2)}, have $${account.buying_power.toFixed(2)}`);
      }
      ...
      const phases = typeof template.phases === 'string' 
        ? JSON.parse(template.phases) 
        : template.phases;
      
      let sharesAllocated = 0;
      for (const phase of phases) {
        const sharesToSell = phase.phase === 4 
      ...
      await NotificationService.send({
        ...
        message: `Entry order placed for ${totalShares} shares at $${entryPrice.toFixed(2)}\nPosition Size: $${calculatedPositionSize.toFixed(2)}\nTemplate: ${template.name}`,
        tradeId: tradeId
      });
  ...
  async handleEntryFill(tradeId, fillPrice, filledQty) {
    ...
    logger.phase('Entry filled, initiating Phase 1', { tradeId, fillPrice, filledQty });
    ...
    try {
      ...
      await NotificationService.send({
        ...
        message: `${filledQty} shares filled at $${fillPrice.toFixed(2)}\nPhase 1 orders placed`,
        tradeId
      });
  ...
  async placePhaseOrders(tradeId, phaseNumber) {
    ...
    try {
      ...
      if (!trade || !phase) {
        throw new Error(`Trade or phase not found: ${tradeId}, phase ${phaseNumber}`);
      }
  ...
  async handlePhaseTakeProfitHit(tradeId, phaseNumber, fillPrice) {
    ...
    try {
      ...
      // Calculate phase P&L
      const phasePnl = (fillPrice - trade.entry_price) * currentPhase.shares_to_sell;
      ...
      const newRemainingShares = trade.remaining_shares - currentPhase.shares_to_sell;
      await db('trades')
        .where('id', tradeId)
        .update({ 
      ...
      const oldSlOrders = await db('orders')
        .where({ trade_id: tradeId, phase: phaseNumber, purpose: 'remaining_sl' });
  ...
  async handlePhaseStopLossHit(tradeId, phaseNumber, fillPrice, filledQty) {
    ...
    logger.phase('Stop loss hit', { tradeId, phaseNumber, fillPrice, filledQty });
  ...
  async completeTrade(tradeId, exitReason) {
    ...
    try {
      ...
      const totalPnl = phases.reduce((sum, p) => sum + parseFloat(p.phase_pnl || 0), 0);
    ...
    try {
      ...
      for (const order of orders) {
        try {
          ...
          logger.warn(`Failed to cancel order ${order.alpaca_order_id}:`, e.message);
        }
        await db('orders')
          .where('id', order.id)
          .update({ status: 'cancelled' });
